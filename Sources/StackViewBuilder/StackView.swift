...
public protocol StackViewProtocol: UIView {
    ...
    @discardableResult func stackedViews(@StackViewBuilder _ views: () -> [StackableView]) -> Self
...
extension StackViewProtocol {
    public func stackedViews(@StackViewBuilder builder: () -> [StackableView]) -> Constrainable {
        configure { view in
            view.stackedViews(builder)
        }
    }

    @discardableResult public func spacing(_ spacing: CGFloat) -> Constrainable {
        configure {
            $0.spacing(spacing)
        }
    }

    @discardableResult public func spacing(_ spacing: CGFloat, after: StackableView) -> Constrainable {
        configure {
            $0.spacing(spacing, after: after)
        }
    }
}
...
public class StackView: UIView, StackViewProtocol {
    ...
    public init(axis: NSLayoutConstraint.Axis = .vertical, @StackViewBuilder subviews: () -> [StackableView] = { [] }) {
